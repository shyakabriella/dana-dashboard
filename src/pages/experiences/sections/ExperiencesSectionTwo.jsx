...
export default function ExperiencesSectionTwo() {
  const [sectionData, setSectionData] = useState({
    id: null,
    title: "— BEGIN YOUR STORY",
    subtitle: "Reserve your first experience.",
    description: "Many experiences are exclusive to guests. Book a room and unlock the full ridge.",
    button_one_text: "Reserve a Stay",
    button_two_text: "View Rooms",
  });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/dana/experiences/section-two`, {
      ...
      if (result.success && result.data && result.data.length > 0) {
        ...
        setSectionData({
          id: section.id,
          title: section.title || "— BEGIN YOUR STORY",
          subtitle: section.subtitle || "Reserve your first experience.",
          description: section.description || "Many experiences are exclusive to guests. Book a room and unlock the full ridge.",
          button_one_text: section.button_one_text || "Reserve a Stay",
          button_two_text: section.button_two_text || "View Rooms",
        });
        setHasChanges(false);
      }
  ...
  const saveToBackend = async () => {
    ...
    try {
      ...
      const payload = { title: sectionData.title, subtitle: sectionData.subtitle, description: sectionData.description, button_one_text: sectionData.button_one_text, button_two_text: sectionData.button_two_text };
      let url, method;
      if (sectionData.id) { url = `${API_URL}/dana/experiences/section-two/${sectionData.id}`; method = "PUT"; }
      else { url = `${API_URL}/dana/experiences/section-two`; method = "POST"; }
    ...
    <div className="space-y-6">
      <div className="rounded-2xl border bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div><h2 className="text-xl font-bold text-gray-900">Section 2 - Begin Your Story</h2><p className="text-sm text-gray-500">Edit the call to action section</p></div>
      ...
      <div className="rounded-2xl border bg-white p-6 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-4 mt-4">
          <div><label className="mb-1 block text-sm font-medium">Button 1 Text</label><input type="text" value={sectionData.button_one_text} onChange={(e) => updateField("button_one_text", e.target.value)} className="w-full rounded-lg border px-3 py-2 focus:border-amber-400 focus:outline-none" /></div>
          <div><label className="mb-1 block text-sm font-medium">Button 2 Text</label><input type="text" value={sectionData.button_two_text} onChange={(e) => updateField("button_two_text", e.target.value)} className="w-full rounded-lg border px-3 py-2 focus:border-amber-400 focus:outline-none" /></div>
        </div>
      </div>
      ...
      <div className="rounded-2xl border bg-white p-6 shadow-sm">
        ...
        <div className="bg-gradient-to-r from-amber-800 to-amber-600 rounded-xl p-12 text-center text-white">
          ...
          <p className="text-white/80 max-w-2xl mx-auto mb-8">{sectionData.description}</p>
          <div className="flex flex-wrap justify-center gap-4">
            <button className="bg-white text-amber-700 px-8 py-3 rounded-lg font-semibold hover:bg-gray-100 transition">{sectionData.button_one_text}</button>
            <button className="border border-white text-white px-8 py-3 rounded-lg font-semibold hover:bg-white/10 transition">{sectionData.button_two_text}</button>
          </div>
        </div>
      </div>
    </div>
